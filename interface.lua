...
local library = {tabs = {}, draggable = true, flags = {}, title = "Decayed", open = false, mousestate = inputService.MouseIconEnabled, popup = nil, instances = {}, connections = {}, options = {}, notifications = {}, tabSize = 0, theme = {}, foldername = "decayed", fileext = ".json"}
...
getgenv().synlog = NEON:github('belkworks', 'synlog')

synlog:setDefaultTime(1)
...
function library:Notification(type, message, ...)
	local arguments = {...}

	synlog[type](synlog, message, unpack(arguments))
end
...
function library:Unload()
	...
	library:Notification("print", "Unloaded in", synlog.chalk(tostring(math.ceil((tick() - start) * 1000)).. "ms").light.red)
...
function library:GetConfigs()
	if not isfolder(self.foldername) then
		...
		makefolder(self.foldername .. "/lua")
	end
	local files = listfiles(self.foldername)
	for i,v in next, files do
		if v:find(self.fileext) then
			v = v:gsub(self.foldername .. "\\", "")
			files[i] = v:gsub(self.fileext, "")
		else
			files[i] = nil
		end
	end
...
function library:GetLuas()
	local files = listfiles(self.foldername .. "/lua")
	for i,v in next, files do
		if v:find(".lua") then
			v = v:gsub(self.foldername .. "/lua" .. "\\", "")
			files[i] = v:gsub(".lua", "")
		else
			files[i] = nil
		end
...
local function createButton(option, parent)
	...
	option.title = library:Create("TextLabel", {
		...
		BorderColor3 = Color3.new(),
		Text = option.text,
		TextColor3 = (not option.unsafe and Color3.new(1, 1, 1) or Color3.new(1, 1, 0.5)),
		TextSize = 15,
		Font = Enum.Font.Code,
		Parent = option.main
	})
	...
	function tab:AddColumn()
		...
		function column:AddSection(title)
			...
			function section:AddButton(option)
				...
				option.callback = typeof(option.callback) == "function" and option.callback or function() end
				option.type = "button"
				option.unsafe = option.unsafe or false
				option.position = #self.options
				option.flag = (library.flagprefix and library.flagprefix .. " " or "") .. (option.flag or option.text)
				option.subcount = 0
				option.canInit = (option.canInit ~= nil and option.canInit) or true
		...
		if self.open then
			if input.UserInputType.Name == "MouseMovement" then
				if self.cursor then
                    ...
                    self.cursor.PointA = mouse;
                    self.cursor.PointB = mouse + Vector2.new(16,5);
                    self.cursor.PointC = mouse + Vector2.new(5,16);
                    self.cursor1.PointA = self.cursor.PointA + Vector2.new(0, 0)
                    self.cursor1.PointB = self.cursor.PointB + Vector2.new(1, 1)
                    self.cursor1.PointC = self.cursor.PointC + Vector2.new(1, 1)
				end
				if self.slider then
...
do
	...
	-- Theme
	library.SettingsMenu = library.SettingsColumn:AddSection("Theme");
	do
		...
		library.SettingsMenu:AddList({
			text = "Background", 
			flag = "UI Background", 
			values = {"Floral", "Flowers", "Circles", "Hearts", "Chubs"}, 
	...
	-- Configuration
	library.ConfigSection = library.SettingsColumn1:AddSection("Configuration");
	do
		...
		library.ConfigSection:AddButton({
			...
			callback = function()
				...
				for i, config in next, library.options["Config List"].values do
					if not table.find(Configs, config) then
						library.options["Config List"]:RemoveValue(config)
					end
				end
		...
		-- Luas
		library.ConfigSection:AddDivider("Luas");
		library.ConfigSection:AddBox({
			text = "Lua Name", 
			skipflag = true
		});
		library.ConfigSection:AddList({
			...
			value = "", 
			flag = "Lua List", 
			values = library:GetLuas()
		})
		library.ConfigSection:AddButton({
			text = "Load", 
			callback = function()
				loadstring(readfile(library.foldername .. "/lua" .. "/" .. library.flags["Lua List"] .. ".lua"))();
			end
		});
		library.ConfigSection:AddButton({
			text = "Delete", 
			callback = function()
				local Config = library.flags["Lua List"]
				library.options["Lua List"]:RemoveValue(Lua)
				delfile(library.foldername .. "/lua/" .. Lua .. ".lua")
			end
		});
	end;
end;
